...
export function activate(context: vscode.ExtensionContext) {
	...
	let disposable = vscode.commands.registerCommand('lightweight-graphviz.preview', () => {
		if (!preview || !preview.IsValid())
			preview = new LWPreview();
		preview.Refresh();
	});
	let disposable2 = vscode.commands.registerCommand('lightweight-graphviz.export', () => {
		let fileexport = new LWFileExport()
		fileexport.Save();
	});
